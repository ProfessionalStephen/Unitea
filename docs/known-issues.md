...
| KI-013 | Pipedrive board mapping | High | open | Deal Tracking may be an unmapped Pipedrive pipeline for active job drilldowns. | User reported the dashboard still showed only 9 active jobs and 1 stuck after changing the API key, and noted this should be the deal tracking area. The dashboard now flags active deals that are outside the configured board mirror, but drilldowns still require exact pipeline/stage mapping. | Run a read-only Pipedrive drift inspection for the new API user. If `Deal Tracking` is a real source board, add it to `shared/domain/boards.ts` with byte-for-byte stage names and tests. |
